...
/*  Student's Name: Sam Essery	2130654	*/

    int preval (FILE *f)
    {


        int x = gettoken(f);                         //get the char from string ans assign to x
        if (x<0){                                    //if char is an operand
                              //
       switch (-i)                                     //switch the char into an operand
       {
           case '-' : return  preval(f) - preval(f);    // go to subtraction
           case '+' : return  preval(f) + preval(f);    // go to addition
           case '*' : return  preval(f) * preval(f);   //go to multiplication
           case '/' : return  preval(f) / preval(f);   //go to division
       }
       return i;
       }
    }
